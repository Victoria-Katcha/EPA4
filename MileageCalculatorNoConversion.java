 ...
 * Package: ch16
 * @author Christopher Williams , Victoria Katcha, jocelyn Varghese
   ...
   // private ToggleGroup tgConv = new ToggleGroup();
    ...
    public void start(Stage primaryStage) {  
     ...
     // create UI components
     		ObservableList<String> items = FXCollections.observableArrayList(defaultResult, altMileage);
        cbox.getItems().addAll(items);
        cbox.setValue(defaultResult);
        
    	// set toggle group for RadioButtons        delete this   
    ...
    //    rbMPG.setSelected(true);
     ...
     //   mainPane.add(rbKPL, 1, 1);
        ...
        cbox.getSelectionModel().selectedItemProperty().addListener(e -> changeLabels());
    ...
    private void changeLabels() {
    	...
    	if (cbox.getValue().equals(cbox.getItems().get(0))) {
        	// update labels
    ...
    private void calcMileage() {       
        ...
        double result = 0.0;
        if (cbox.getValue().equals(cbox.getItems().get(1))) {
        	// liters / 100KM
    ...
    private void resetForm() {
    	...
    	cbox.getValue().equals(cbox.getItems().get(0));
    	cbox.setValue(defaultResult);
